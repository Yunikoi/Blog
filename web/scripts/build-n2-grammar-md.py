...
GRAMMAR_RULES: list[tuple[str, str, str]] = [
    ...
    (r"ほかない|しかない|よりほか|にしか|からしか|だけしか", "〜しかない／ほかない", "限定"),
    ...
    (r"申し上げ|おっしゃ|いただ|くださ|うけたまわ|お越し|参り|伺い|差し上げ|お聞き|かしこま|差し支え", "敬语", "敬语"),
    ...
    (r"にしては|としては|にしても|としても|であれ|であっても|にしろ|にせよ|とはいえ|たとえ|ても|離れても", "让步", "让步"),
    ...
    (r"いちばん|よりも|ほうが|わりに|に比べ", "比较表达", "关系"),
]
...
def build_guide_section(tag_count: Counter[str], tag_exams: dict[str, set[str]]) -> list[str]:
    ...
    tags_sorted = [
        t for t, _ in tag_count.most_common()
        if t not in ("（待归类）", "★语序题")
    ]
    if "★语序题" in tag_count:
        tags_sorted.append("★语序题")
    if "（待归类）" in tag_count:
